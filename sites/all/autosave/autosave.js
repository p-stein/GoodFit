  ...
  attach: function (context, settings) { 
    ...
    if (autosaved.serialized) {
      ...
      $('#autosave-status #view a').click(function() {
		...
		var autosaved_serialized = parseUri(autosaved.serialized);
		console.log( autosaved_serialized );

		alert(autosaved_serialized["field_agerange[und]"]);

          $('#' + autosaved_form_id).formHash( parseUri(autosaved.serialized) );
  ...
  saveForm: function() {
    ...
    $.ajax({
      ...
      data: ({
        ...
        q: autosaved.q,
        serialized: jQuery.param(serialized)
      }),
